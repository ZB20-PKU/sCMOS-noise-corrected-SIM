...
%     weightStack(:,:,:)=1;
% end
% % imwritestack(weightStack, 'weightStack.tif')
...
b = compute_b(rawDataFFT, illuminationPatternConj, otfConjStack);
Ax0 = compute_Ax(ReconImg, illuminationPattern, illuminationPatternConj, ...
...
for i = 1: iterationTime
    ...
    vi = compute_Ax(pI, illuminationPattern, illuminationPatternConj, ...
                otfStack, otfConjStack, weightStack,...
                highPassFilter, hpPara, wienerFilerPara);
    alpha = roui/(sum(sum(r0_hat.* vi)));
    s = ri_1 - alpha*vi;
    t = compute_Ax(s, illuminationPattern, illuminationPatternConj, ...
                otfStack, otfConjStack, weightStack,...
                highPassFilter, hpPara, wienerFilerPara);
...
saveName = ['ReconWegImgStack w' num2str(wienerFilerPara) ' error' num2str(stopTH)];
if avgFlag == 1
    saveName = ['ReconAvgImgStack w' num2str(wienerFilerPara) ' error' num2str(stopTH)];
end
% saveName = ['weg-hpfilter'];
saveName = ['weg'];
